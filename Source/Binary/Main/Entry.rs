...
impl Entry {
    ...
    async fn run_standalone(args: CliArgs) -> Result<()> {
        ...
        let host_config = HostConfig::default()
            .with_activation_timeout(args.max_execution_time_ms);
    ...
    pub async fn validate_extension(
        ...
        if !path.exists() {
            return Ok(ValidationResult {
                is_valid: false,
                errors: vec![format!("Path does not exist: {:?}", path)],
            });
        }
    ...
    fn create_transport(args: &CliArgs) -> Result<Transport> {
        match args.transport.as_str() {
            ...
            "ipc" => Ok(Transport::Ipc(
                crate::Transport::IpctTransport::new()
                    .context("Failed to create IPC transport")?,
            )),
            "wasm" => Ok(Transport::Wasm(
...
mod tests {
    ...
    async fn test_validate_extension_nonexistent() {
        let result = Entry::validate_extension(
