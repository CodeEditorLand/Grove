...
use CommonLibrary::{
    Transport::{
        TransportConfig, TransportError, TransportMetrics, TransportStrategy, UnifiedRequest,
        UnifiedResponse, TransportCapabilities, TransportType as CommonTransportType,
    },
    Environment::Environment,
};

use crate::Transport::{
    GrpcTransport as GroveGrpcTransport, IPCTransportImpl as GroveIpcTransport,
    WASMTransportImpl as GroveWasmTransport, Transport as GroveTransport,
    TransportConfig as GroveTransportConfig, TransportStats as GroveTransportStats,
    TransportStrategy as GroveTransportStrategy, TransportType as GroveTransportType,
};
...
impl TransportAdapter {
    ...
    pub fn grove_transport(&self) -> &GroveTransport {
        &self.transport
    }

    /// Gets the transport configuration.
    pub fn config(&self) -> &GroveTransportConfig {
        &self.config
    }
    ...
    fn translate_metrics(stats: GroveTransportStats) -> TransportMetrics {
        TransportMetrics {
            ...
            latency_ms_histogram: stats.avg_latency_us.map(|us| (1, us as f64 / 1000.0, (us as f64 / 1000.0).powi(2))),
...
impl TransportStrategy for TransportAdapter {
    async fn connect(&mut self) -> Result<(), TransportError> {
        self.transport.connect().await.map_err(|e| {
            TransportError::connection(format!("Failed to connect transport: {}", e))
                .with_transport_type("grove")
        })
    }

    async fn disconnect(&mut self) -> Result<(), TransportError> {
        self.transport.close().await.map_err(|e| {
            TransportError::connection(format!("Failed to disconnect transport: {}", e))
                .with_transport_type("grove")
        })
    }
        ...
        match result {
            Ok(response_bytes) => {
                let correlation_id = request
                    .correlation_id
                    .unwrap_or_else(|| (self.correlation_generator)());
            ...
            Err(e) => {
                ...
                Ok(UnifiedResponse::from_transport_error(
                    correlation_id,
                    &e,
                ))
            }
    ...
    async fn send_notification(
        ...
        notification.validate().map_err(|e| {
            TransportError::invalid_request(format!("Invalid notification: {}", e))
                .with_transport_type("grove")
        })?;
    ...
    fn is_connected(&self) -> bool {
        // Unfortunately Grove's `is_connected` requires `&self`, not `&self` with interior mutability?
        // Let's check the trait again - it takes `&self` which is fine.
        // But `Transport` enum's `is_connected` method takes `&self`, which is also fine.
        // However, we have `Arc<Transport>`, so we can call it.
        // Wait, `Transport` enum's `is_connected` method is defined on `&self` and doesn't require mut.
        // But we have `transport: Arc<GroveTransport>`, we can call `self.transport.is_connected()`.
        // But in the method signature above we have `self.transport` as `Arc<GroveTransport>`, not `&GroveTransport`.
        // We need to deref.
        // Let's check: `self.transport.is_connected()` should work because `Arc<GroveTransport>` derefs to `GroveTransport`.
        // But `is_connected` takes `&self`. `Arc` deref to `T`, so `&self.transport` is `&&GroveTransport`, that's fine.
        // Actually `self.transport.is_connected()` means we're calling `is_connected` on `Arc<GroveTransport>`, which
        // uses deref coercion to call `is_connected` on `&GroveTransport`? Let's think: `Arc<T>` implements `Deref<Target = T>`,
        // so `&Arc<T>` can be used as `&T`. But here we have `self.transport` which is `Arc<GroveTransport>`, not `&Arc<...>`.
        // So we need to either do `(&*self.transport).is_connected()` or simply `self.transport.is_connected()` doesn't work
        // because `is_connected` expects `&self` and we have an owned `Arc`. Actually `Arc<T>` does not automatically deref
        // to `&T` when you have an owned `Arc`. You need to pass a reference: `(&*self.transport).is_connected()`.
        // So I'll fix: `(&*self.transport).is_connected()`
    ...
    fn config(&self) -> &TransportConfig {
        ...
        // This is tricky because they are different types. We could create a Common TransportConfig
        // from Grove's config. For simplicity, we'll return a dummy common config.
        // In a real implementation, we'd store a Common TransportConfig separately or
        // adapt it. For now, let's create a translated one.
        // Actually we don't have a Common TransportConfig stored. We could create on-the-fly.
        ...
        // Actually the method signature is `&self -> &TransportConfig`, so we need to have it stored.
        // I'll need to adjust the struct to hold an `Arc<TransportConfig>` or store it directly.
        // But I'm not storing it currently. Let's revise the struct to include a common config.
        // I'll need to rewrite the struct. For now, I'll return a reference to an empty static config
        // via lazy_static or something. But simpler: I'll add a field `common_config` to the adapter.
        // Since we want the adapter to be usable, let's modify the struct.
        // I'll do a rewrite of this file with a proper storage of common config.
    ...
    fn metrics(&self) -> TransportMetrics {
        // This is async in Grove but sync here. We can't block. We could spawn a blocking task
        // but that would require runtime. For now, return empty or cached metrics.
        // We could store metrics in an Arc<Mutex> that we update periodically.
        // That's more complex. For a simple adapter, we might change the trait method to async,
        // but it's defined as sync. Let's return a default.
...
impl From<crate::Transport::GrpcTransportError> for TransportError {
    fn from(err: crate::Transport::GrpcTransportError) -> Self {
        match err {
            crate::Transport::GrpcTransportError::ConnectionFailed(msg) => {
                TransportError::connection(msg)
            }
            crate::Transport::GrpcTransportError::NotConnected => {
                TransportError::connection("Not connected")
            }
            ...
            crate::Transport::GrpcTransportError::Timeout => {
...
// TODO: Implement From<IPCTransportError> and From<WasmTransportError> when those are defined
...
mod tests {
    ...
    async fn test_adapter_creation() {
        let grove_transport = GroveTransport::gRPC(
            GrpcTransport::new("127.0.0.1:50050").unwrap()
        );
