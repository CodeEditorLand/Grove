...
pub use IpctTransport::IpcTransport;
...
pub fn create_default_transport() -> Transport {
    Transport::default()
}

/// Create a gRPC transport with the given address
pub fn create_grpc_transport(address: &str) -> Result<Transport> {
    Ok(Transport::Grpc(GrpcTransport::new(address)?))
}

/// Create an IPC transport
pub fn create_ipc_transport() -> Result<Transport> {
    Ok(Transport::Ipc(IpcTransport::new()?))
}
