...
use crate::Transport::Strategy::TransportStats;
use crate::Transport::TransportConfig;
use crate::WASM::{HostBridge, MemoryManager::{MemoryManager, MemoryLimits}, Runtime::{WasmConfig, WasmRuntime}, WasmStats};
...
impl WasmTransport {
    ...
    pub fn new(
        ...
        // Create runtime - this would normally be async, but for now we do it synchronously
        // In production, this would need to be properly awaited
        ...
        let memory_limits = MemoryLimits::new(
            memory_limit_mb,
            (memory_limit_mb as f64 * 0.75) as u64,
            100,
        );
    ...
    pub async fn call_wasm_function(
        ...
        if let Some(module) = modules_mut.get_mut(module_id) {
            let stats = module.function_stats.entry(function_name.to_string())
                .or_default();
...
impl super::super::Strategy::TransportStrategy for WasmTransport {
    ...
    async fn send(&self, request: &[u8]) -> Result<Vec<u8>, Self::Error> {
        ...
        if parts.len() < 3 {
            return Err(WasmTransportError::InvalidRequest(
    ...
    fn is_connected(&self) -> bool {
        self.connected.blocking_read().to_owned()
    }
}
