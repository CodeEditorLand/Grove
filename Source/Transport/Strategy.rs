...
impl Transport {
        ...
        match self {
            Self::Grpc(transport) => {
                transport
                    .close()
                    .await
                    .map_err(|e| anyhow::anyhow!("gRPC close error: {}", e))
            }
            Self::Ipc(transport) => {
                transport
                    .close()
                    .await
                    .map_err(|e| anyhow::anyhow!("IPC close error: {}", e))
            }
            Self::Wasm(transport) => {
                transport
                    .close()
                    .await
                    .map_err(|e| anyhow::anyhow!("WASM close error: {}", e))
            }
        }
    }
    ...
    pub fn as_ipc(&self) -> Option<&super::IpctTransport> {
        match self {
...
impl fmt::Display for Transport {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "Transport({})", self.transport_type())
    }
}
...
mod tests {
    ...
    fn test_transport_type_from_str() {
        ...
        assert_eq!(
            "ipc".parse::<TransportType>().unwrap(),
            TransportType::Ipc
        );
        assert_eq!(
            "wasm".parse::<TransportType>().unwrap(),
            TransportType::Wasm
        );
