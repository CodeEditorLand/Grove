...
use crate::Transport::{DEFAULT_CONNECTION_TIMEOUT_MS, DEFAULT_REQUEST_TIMEOUT_MS, TransportConfig};
...
impl From<tonic::transport::Error> for GrpcTransportError {
    fn from(err: tonic::transport::Error) -> Self {
        GrpcTransportError::ConnectionFailed(err.to_string())
    }
}

impl From<tonic::Status> for GrpcTransportError {
    fn from(status: tonic::Status) -> Self {
        GrpcTransportError::GrpcError(status.to_string())
    }
}
