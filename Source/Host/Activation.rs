...
use crate::Host::{ExtensionManager, HostConfig, ActivationResult};
use crate::Common::traits::ExtensionContext;
    ...
    pub fn from_str(event_str: &str) -> Result<Self> {
        match event_str {
            ...
            e if e.starts_with("onCommand:") => {
                Ok(Self::Command(e.trim_start_matches("onCommand:").to_string()))
            }
            e if e.starts_with("onLanguage:") => {
                Ok(Self::Language(e.trim_start_matches("onLanguage:").to_string()))
            }
            ...
            e if e.starts_with("onView:") => {
                Ok(Self::OnView(e.trim_start_matches("onView:").to_string()))
            }
            e if e.starts_with("onUri:") => {
                Ok(Self::OnUri(e.trim_start_matches("onUri:").to_string()))
            }
            e if e.starts_with("onFiles:") => {
                Ok(Self::OnFiles(e.trim_start_matches("onFiles:").to_string()))
            }
...
impl ActivationEngine {
    ...
    pub async fn activate(&self, extension_id: &str) -> Result<ActivationResult> {
        ...
        let activation_events: Result<Vec<ActivationEvent>> =
            extension_info
                .activation_events
                .iter()
                .map(|e| ActivationEvent::from_str(e))
                .collect();
        let activation_events = activation_events
            .with_context(|| "Failed to parse activation events")?;

        // Create activation context
        let context = ActivationContext::default();

        // Perform activation (in real implementation, this would call the extension's activate function)
...
mod tests {
    ...
    fn test_activation_event_to_string() {
        ...
        assert_eq!(
            ActivationEvent::Command("test".to_string()).to_string(),
            "onCommand:test"
        );
        assert_eq!(
            ActivationEvent::Language("rust".to_string()).to_string(),
            "onLanguage:rust"
        );
    }
