...
impl APIBridge {
    ...
    pub async fn handle_call(&self, request: APICallRequest) -> Result<APICallResponse> {
        ...
        // Execute the API method (in real implementation, this would call the actual handler)
        let result = self
        ...
        if exists {
            ...
            stats.avg_latency_us = (stats.avg_latency_us * (stats.successful_calls - 1) + elapsed_us) / stats.successful_calls;
        }
        ...
        debug!(
            "API call {} completed in {}µs",
            request.api_method, elapsed_us
        );
