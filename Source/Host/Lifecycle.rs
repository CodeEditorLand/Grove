...
impl LifecycleManager {
    ...
    pub async fn transition(
        ...
        self.perform_state_transition(extension_id, event, new_state)
            .await?;
    ...
    fn determine_next_state(
        ...
        match (current_state, event) {
            (LifecycleState::Created, LifecycleEvent::Initialize) => {
    ...
    pub async fn get_event_history_for_extension(
        &self,
        extension_id: &str,
...
mod tests {
    ...
    async fn test_lifecycle_manager_registration() {
        ...
        let result = manager
            .register_extension("test.ext", LifecycleState::Created)
            .await;
        
        assert!(result.is_ok());
        assert_eq!(
            manager.get_state("test.ext").await,
    ...
    async fn test_state_transitions() {
        ...
        // Initialize
        let state = manager
            .transition("test.ext", LifecycleEvent::Initialize)
            .await
            .unwrap();
        ...
        // Start
        let state = manager
            .transition("test.ext", LifecycleEvent::Start)
            .await
            .unwrap();
        ...
        // Stop
        let state = manager
            .transition("test.ext", LifecycleEvent::Stop)
            .await
            .unwrap();
