//! Host Bridge
//!
//! Provides bidirectional communication between the host (Grove) and WASM modules.
//! Handles function calls, data transfer, and marshalling between the two environments.
...
pub type AsyncHostFunctionCallback = fn(Vec<Bytes>) -> Box<dyn std::future::Future<Output = Result<Bytes>> + Send + Unpin>;
...
impl HostBridge {
    ...
    pub async fn call_host_function(
        ...
        if let Some(callback) = func.callback {
            // Synchronous call
            let result = callback(args).map_err(|e| {
                BridgeError::HostFunctionError(format!("{}: {}", function_name, e))
            })?;
    ...
    pub async fn receive_from_wasm(&mut self) -> Option<WasmMessage> {
        self.wasm_to_host_rx.recv().await
    }
...
mod tests {
    ...
    async fn test_register_host_function() {
        ...
        let result = bridge
            .register_host_function("echo", signature, |args| {
                Ok(args[0].clone())
            })
            .await;
    ...
    fn test_marshal_unmarshal() {
        let args = vec![
            serialize_to_bytes(&42i32).unwrap(),
            serialize_to_bytes(&3.14f64).unwrap(),
        ];
