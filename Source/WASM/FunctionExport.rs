...
use crate::WASM::HostBridge::{HostBridge, HostFunctionCallback, FunctionSignature, ParamType, ReturnType};
...
impl FunctionExport {
    ...
    pub fn new(bridge: Arc<HostBridge>) -> Self {
        Self {
            registry: Arc::new(HostFunctionRegistry {
    ...
    pub async fn register_functions(
        ...
        if signatures.len() != callbacks.len() {
            return Err(anyhow::anyhow!(
                "Number of signatures must match number of callbacks"
            ));
        }
    ...
    where
        T: Send + 'static,
    {
        info!("Exporting {} host functions to linker", 
        ...
        T: Send + 'static,
    {
        ...
        let callback = func.callback.ok_or_else(|| {
            anyhow::anyhow!("No callback available for function: {}", name)
        })?;
        ...
        let wrapped_callback = move |mut caller: Caller<'_, T>, args: &[wasmtime::Val]| -> Result<Vec<wasmtime::Val>, wasmtime::Trap> {
            let start = std::time::Instant::now();
            
            // Convert args to bytes
            let args_bytes: Result<Vec<bytes::Bytes>, _> = args.iter()
                .map(|arg| {
                    match arg {
                        wasmtime::Val::I32(i) => {
                            serde_json::to_vec(i).map(bytes::Bytes::from)
                        ...
                        wasmtime::Val::F32(f) => {
                            serde_json::to_vec(f).map(bytes::Bytes::from)
                                .map_err(|e| anyhow::anyhow!("Serialization error: {}", e))
                        }
                        wasmtime::Val::F64(f) => {
                            serde_json::to_vec(f).map(bytes::Bytes::from)
            ...
            let args_bytes = args_bytes.map_err(|e| {
                wasmtime::Trap::new(format!("Argument conversion failed: {}", e))
            })?;
            ...
            match result {
                Ok(response_bytes) => {
                    ...
                    let result_val: serde_json::Value = serde_json::from_slice(&response_bytes)
                        .map_err(|e| {
                            wasmtime::Trap::new(format!("Response deserialization failed: {}", e))
                        })?;
...
mod tests {
    ...
    async fn test_register_function() {
        ...
        let callback = |args: Vec<bytes::Bytes>| {
            Ok(args.get(0).cloned().unwrap_or(bytes::Bytes::new()))
        };
