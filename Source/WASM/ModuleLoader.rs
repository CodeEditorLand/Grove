...
use crate::WASM::{HostBridge, MemoryManager};
...
impl ModuleLoader {
    ...
    pub async fn load_from_url(&self, url: &str) -> Result<WasmModule> {
        ...
        if !response.status().is_success() {
            return Err(anyhow::anyhow!(
                "Failed to fetch WASM module: HTTP {}",
                response.status()
            ));
        }
    ...
    pub async fn get_loaded_modules(&self) -> Vec<WasmModule> {
        self.loaded_modules.read().await.clone()
    }
        ...
        for export in module.exports() {
            match export.ty() {
                ...
                wasmtime::ExternType::Memory(_) => {
                    exports.memories.push(export.name().to_string())
                }
